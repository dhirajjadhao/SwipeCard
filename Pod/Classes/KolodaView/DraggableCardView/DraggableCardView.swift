...
private let defaultDisableVerticalSwipe: Bool = false
...
public class DraggableCardView: UIView, UIGestureRecognizerDelegate {
    ...
    public var disableVerticalSwipe = defaultDisableVerticalSwipe
    ...
    @objc func panGestureRecognized(_ gestureRecognizer: UIPanGestureRecognizer) {
        ...
        case .began:
            ...
            let newAnchorPoint = CGPoint(x: firstTouchPoint.x / bounds.width, y: disableVerticalSwipe ? 0 : (firstTouchPoint.y / bounds.height))
            let oldPosition = CGPoint(x: bounds.size.width * layer.anchorPoint.x, y: disableVerticalSwipe ? 0 : (bounds.size.height * layer.anchorPoint.y))
            let newPosition = CGPoint(x: bounds.size.width * newAnchorPoint.x, y: disableVerticalSwipe ? 0 : (bounds.size.height * newAnchorPoint.y))
            layer.anchorPoint = newAnchorPoint
            layer.position = CGPoint(x: layer.position.x - oldPosition.x + newPosition.x, y: disableVerticalSwipe ? 0 : (layer.position.y - oldPosition.y + newPosition.y))
            removeAnimations()
...
//            transform = CATransform3DScale(transform, scale, scale, 1)
            ...
            transform = CATransform3DTranslate(transform, dragDistance.x, disableVerticalSwipe ? 0 : dragDistance.y, 0)
